...
module.exports = {
    ...
    aliases: ['h'],
    category: 'Fun',
    utilisation: '{prefix}player [player name]',
    execute(client, message, args) {
        //check that a value is sent
        if(!args.length) {
           return message.channel.send('please specify the player name');
        }
        //get uuid
        this.getUuid(args[0], (err, uuid) => {
            if(err) {
               return message.channel.send('An error occurred. This might be because the player does not exist');
            }
