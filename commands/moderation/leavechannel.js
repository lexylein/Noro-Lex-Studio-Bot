...
module.exports = {
    ...
    aliases: [],
    category: 'Moderation',
    utilisation: '{prefix}leavechannel [Message]',
	execute(client, message, args) {
        if (!message.member.hasPermission("ADMINISTRATOR")) {
				...
				db.set(`leavechannel_${message.guild.id}`, newStr)
				let leavechanneldb = db.fetch(`leavechannel_${message.guild.id}`)
				message.channel.send(`the new leave channel is: ${leavechanneldb}`)
	},
};